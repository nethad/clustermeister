...
public class JPPFLocalNode {
    ...
    protected void prepareNodeConfiguration() throws IOException {
        ...
        //TODO:  options setting from config should propagate here
        properties.setProperty(JPPFConstants.JVM_OPTIONS, 
                "-Xms64m -Xmx512m -Djava.util.logging.config.file=config/logging-node.properties");
        ...
        properties.setProperty(JPPFConstants.RESOURCE_CACHE_DIR, 
                "/tmp/.jppf/node-" + System.currentTimeMillis());
        ...
        final File propertiesFile = new File(configDir, nodeConfigFileName);
        
        BufferedWriter fileWriter = Files.newWriter(propertiesFile, Charsets.UTF_8);
        try {
            properties.store(fileWriter, "JPPF-node configuration generated by Clustermeister.");
        } finally {
            if(fileWriter != null) {
                fileWriter.close();
            }
