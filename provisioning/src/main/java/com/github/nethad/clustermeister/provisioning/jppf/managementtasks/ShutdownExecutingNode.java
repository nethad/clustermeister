 ...
 * @author daniel
 */
public class ShutdownExecutingNode implements Serializable {
    @JPPFRunnable
    public void shutdownNode(String jobId) throws TimeoutException, Exception {
        JMXNodeConnectionWrapper wrapper = new JMXNodeConnectionWrapper();
        wrapper.connect();
        try {
            wrapper.cancelJob(jobId, false);
            wrapper.shutdown();
        } finally {
            wrapper.close();
        }
