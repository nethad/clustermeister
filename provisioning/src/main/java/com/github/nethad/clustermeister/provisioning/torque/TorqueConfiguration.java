...
//torque.email_notify = thomas.ritter@uzh.ch
    ...
    public TorqueConfiguration(String sshUser, String privateKeyPath, String sshHost, int sshPort, String emailNotify) {
    ...
    public static TorqueConfiguration buildFromConfig(Configuration configuration) throws ConfigurationValueMissingException {
        String sshHost = 
                checkString2(configuration, TORQUE_SSH_HOST, "", "You didn't specify an ssh host");
        if (sshHost.isEmpty()) {
            throw new ConfigurationValueMissingException("You have to specify an ssh host.");
        }
        String sshUser = 
                checkString2(configuration, TORQUE_SSH_USER, System.getProperty("user.name"), "You didn't specify a ssh user");
        String defaultPrivateKeyPath = System.getProperty("user.home") + "/.ssh/id_dsa";
        String privateKeyPath = 
                checkString2(configuration, TORQUE_SSH_PRIVATEKEY, defaultPrivateKeyPath, "You didn't specify a private key path");
        String emailNotify = 
                checkString2(configuration, TORQUE_EMAIL_NOTIFY, "", "You didn't specify an email address for notifications");
        int sshPort = checkInt2(configuration, TORQUE_SSH_PORT, 22, "You didn't specify an ssh port");
        return new TorqueConfiguration(sshUser, privateKeyPath, sshHost, sshPort, emailNotify);
    }
    
    private static String checkString2(Configuration configuration, String configOption, String defaultValue, String logMessage) {
        String value = configuration.getString(configOption, "");
        if (value.isEmpty()) {
            logger.warn(loggerMessage2(logMessage, configOption, defaultValue));
            return defaultValue;
        }
        return value;
    }
    
    private static int checkInt2(Configuration configuration, String configOption, int defaultValue, String logMessage) {
        int value = configuration.getInt(configOption, -1);
        if (value == -1) {
            logger.warn(loggerMessage2(logMessage, configOption, String.valueOf(defaultValue)));
            return defaultValue;
        }
        return value;
    }
    
    private static String loggerMessage2(String message, String configOption, String defaultValue) {
