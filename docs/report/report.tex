...
Assessing Clustermeister performance highly depends on the computation task at hand and may be significantly improved by the load-balancing (task distribution) algorithm chosen for the (local) JPPF server. The available load-balancing strategies are\footnote{http://www.jppf.org/doc/v3/index.php?title=Configuring\_a\_JPPF\_server\#Load-balancing}:
...
If the computation involves many short-running (split seconds) tasks, it is favorable to send many such tasks in a batch, even more so if all the tasks combined only take a few seconds to complete. Some of the load-balancing strategies (TODO which?) apply a ``slow start principle'' and perform badly for short-running computations.

\subsection{Deployment Time}

The time to deploy a Clustermeister infrastructure depends on the target platform. Torque cluster nodes are statically configured and are running continuously, hence node start-up time does not need to be accounted for. For amazon instances, however, instance start-up time is significant and does take between 30 and 120 seconds (TODO correct numbers?).

Another facet of the deployment time is the actual \textit{code} deployment time. If you, for example, run code written in Java that uses a lot of libraries, each library class that is used needs to be sent to the nodes. This dynamic class loading happens on demand and depending on the size of the libraries can take a few minutes to complete. Fortunately, JPPF allows for libraries to be preloaded when a node starts up. Clustermeister provides configuration options to specify a Maven pom file to extract the necessary libraries and deploy them with the Clustermeister infrastructure. This way, projects that use Maven integrate well with the Clustermeister framework. Dependencies may be configured manually, refer to section \ref{appendix-general-configuration} for more details.
